#include<bits/stdc++.h>
using namespace std;  // 去掉 #define int long long
...
public:
    ...
    T findKthLargest(int Subscript, int k){  // 返回T类型保持一致
        if(Linetree[Subscript].L==Linetree[Subscript].R){
...
int main(){
    ...
    for(int i=1,k;i<=n;i++){
        ...
        if(opt=='I'){
            ...
            // 修复：正确处理边界条件
            if(threshold > 0){
                int upper_bound = min(threshold-1, mxp-1);  // 确保不越界
                if(upper_bound >= 0){  // 确保查询范围有效
                    int gocnt=tr.query(1, 0, upper_bound);
                    goman+=gocnt;
