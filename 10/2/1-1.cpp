...
vector<vector<int>> edge_k; // 重构树的邻接表
...
void build_kruskal_tree()
{
    ...
    for (auto [w, u, v] : edges)
    {
        ...
        if (fu != fv)
        {
            ...
            fa[fu] = num;
            fa[fv] = num;
            edge_k[num].push_back(fu);
            edge_k[num].push_back(fv);
            val[num] = w;
        }
    }
...
int main()
{
    ...
    // 预处理 LCA
    int max_nodes = n * 2;
    fa_k.assign(max_nodes, vector<int>(20, 0));
    dep.assign(max_nodes, 0);
    dfs(root, 0); // 从根节点开始 DFS
