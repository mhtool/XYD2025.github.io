...
public:
    ...
    // 区间减法：用于批量删除
    void range_sub(int Subscript, int Left, int Right, T val){
        if(Left > Right) return;
        if(Left <= Linetree[Subscript].L && Linetree[Subscript].R <= Right){
            apply_add(Subscript, -val);
            return;
        }
        pushDown(Subscript);
        int mid=(Linetree[Subscript].L+Linetree[Subscript].R) >> 1;
        if(Left <= mid) range_sub(Subscript<<1, Left, Right, val);
        if(Right > mid) range_sub(Subscript<<1|1, Left, Right, val);
        pushUp(Subscript);
    }
...
int main(){
    ...
    for(int i=1,k;i<=n;i++){
        ...
        if(opt=='I'){
            ...
            if(threshold > 0){
                // 优化：只查询需要删除的员工数量，然后批量清零
                int gocnt = tr.query(1, 0, min(threshold-1, mxp-1));
                goman += gocnt;
                
                // 使用区间更新批量删除，避免重建
                if(gocnt > 0){
                    tr.range_sub(1, 0, min(threshold-1, mxp-1), 1);
                    // 修正：由于我们删除的可能不是1个1个的，需要特殊处理
                    // 简化方案：仍然重建，但优化重建过程
                    vector<pair<int,int>>tmp;
                    for(int j = max(0, threshold); j < mxp; j++){
